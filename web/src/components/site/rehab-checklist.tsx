...
export function RehabChecklist({
  items,
  done,
  day,
  history,
  ...
  /**
   * Whether this copy claims the page's `data-first-action` marker (V4 Phase 2++ Stage 7).
   *
   * True on `/private/athletics`, where the ticks genuinely are the only thing you can do. False
   * on the Today mirror, where the capture box already holds that marker — two markers on one
   * page makes `npm run shots` measure whichever it finds last, which is how a passing fold gate
   * starts reporting a number about the wrong element. It reported 1583px the first time this
   * mirror shipped, and the capture box had not moved at all.
   */
  firstAction?: boolean;
}) {
