...
| What Victor is working on *right now* | the `status: active` projects and their `## Updates` sections — same files; published on each project page |
...
| What comes after V4 — AI context, Today/Training cleanup, open audit items | `docs/V5_PLAN.md` |
...
- **Projects are ordered by their `order:` field**, not by tier — tiers were removed on
  2026-08-29 (D-107). `order` is the curated sequence on `/projects` and nothing else — the
  three FRC robots lead it as of 2026-09-23. A new project takes a number and everything below
  it shifts; it does not automatically belong at the bottom. Project `status:` is `active` or
  `done` and now only drives the badge on a card — the `/now` page it used to gate was removed
  on 2026-09-23 (D-342).
- **`## Updates` in a project file is published.** `### YYYY-MM-DD` entries under that heading
  render on the project's own page, newest first — see `web/DECISIONS.md` D-099. Write updates
  for a stranger, not as notes to self; anything internal belongs under `## Notes`, which is
  stripped from every public page.
- **Public pages do not explain the vault.** Copy describing how the site is built — that it
  is generated from markdown, that it publishes itself — was removed on 2026-09-23 (D-343).
  A reader came for the work. Do not reintroduce it.
